...
class SpeechGenerator:
    def __init__(self):
        ...
        # 存储不同语言的声音设置
        self.voice_settings = {
            'en': {'rate': 150, 'voice_id': None},
            'zh': {'rate': 150, 'voice_id': None},
            'ja': {'rate': 150, 'voice_id': None}
        }
    ...
    def _init_tts_engine(self):
        ...
        try:
            if self._check_audio_output():
                self.engine = pyttsx3.init()  # 初始化引擎，可选择sapi5(Windows)、espeak(Linux)、nsss(MacOS)等引擎
